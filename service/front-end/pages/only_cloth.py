...
def main():
    ...
    st.title("의류 가상 피팅 서비스 - Cloth and Avatar")
    ...
    with col2:
        if dress:
            write_category_sub()
            dress_long = st.checkbox("긴소매", value=st.session_state.dress_long, on_change=check_dress_long)
            dress_short = st.checkbox("반소매", value=st.session_state.dress_short, on_change=check_dress_short)
            dress_etc = st.checkbox("민소매", value=st.session_state.dress_etc, on_change=check_dress_etc)
        elif upper:
            write_category_sub()
            upper_long = st.checkbox("긴소매", value=st.session_state.upper_long, on_change=check_upper_long)
            upper_short = st.checkbox("반소매", value=st.session_state.upper_short, on_change=check_upper_short)
            upper_etc = st.checkbox("민소매",value=st.session_state.upper_etc, on_change=check_upper_etc)
        elif lower:
            write_category_sub()
            lower_pantslong = st.checkbox("긴 바지", value=st.session_state.lower_pantslong, on_change=check_lower_pants_long)
            lower_pantsshort = st.checkbox("짧은 바지", value=st.session_state.lower_pantsshort, on_change=check_lower_pants_short)
            lower_skirtlong = st.checkbox("긴 치마", value=st.session_state.lower_skirtlong, on_change=check_lower_skirt_long)
            lower_skirtshort = st.checkbox("짧은 치마", value=st.session_state.lower_skirtshort, on_change=check_lower_skirt_short)
    with col3:
        if st.session_state.dress_long:
            write_category_avatar()
            Woman_a = st.checkbox("여 1", value=st.session_state.Woman_a, on_change=check_woman_one)
            Woman_b = st.checkbox("여 2", value=st.session_state.Woman_b, on_change=check_woman_two)
            Woman_c = st.checkbox("여 3", value=st.session_state.Woman_c, on_change=check_woman_three)
            Woman_d = st.checkbox("여 4", value=st.session_state.Woman_d, on_change=check_woman_four)
            Woman_e = st.checkbox("여 5", value=st.session_state.Woman_e, on_change=check_woman_five)
            ...
            Woman_d = st.checkbox("여 4", value=st.session_state.Woman_d, on_change=check_woman_four)
            Woman_e = st.checkbox("여 5", value=st.session_state.Woman_e, on_change=check_woman_five)
            Man_a = st.checkbox("남 1", value=st.session_state.Man_a, on_change=check_man_one)
            Man_b = st.checkbox("남 2", value=st.session_state.Man_b, on_change=check_man_two)
            Man_c = st.checkbox("남 3", value=st.session_state.Man_c, on_change=check_man_three)
            Man_d = st.checkbox("남 4", value=st.session_state.Man_d, on_change=check_man_four)
            Man_e = st.checkbox("남 5", value=st.session_state.Man_e, on_change=check_man_five)
    ...
    with col2:
        if len(image_path) >= 3:
            avatar_image_path = os.path.join(dir_root, f"service/back-end/sample_images/{image_path[1]}/{image_path[2].split('_')[1]}/{image_path[0]}.jpg")
