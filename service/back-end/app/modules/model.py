...
class MFEBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size=3, num_filters=[128, 64, 32]
    ):
...
class DAFlowNet(nn.Module):
    def __init__(self, num_pyramid, fpn_dim=256, head_nums=1):
        ...
        for i in range(num_pyramid):
            ...
            Self_MFE_layer = MFEBlock(
                in_channels=2 * fpn_dim, out_channels=self.k * 3, kernel_size=7
            )
        ...
        self.lights_encoder = torch.nn.Sequential(
            torch.nn.Conv2d(3, out_channels=32, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
    ...
    def forward(
        ...
        for i in range(len(source_feats)):
            ...
            att_reference_feat = DAWarp(
                feat_ref, last_multi_self_offsets, self_att_maps, self.k, self.out_ch
            )
            ...
            self_offsets = apply_offset(
                offsets_att[:, self.k * 2 : self.k * 4, :, :].reshape(-1, 2, H, W)
            )
            ...
            if return_all:
                ...
                if use_light_en_de:
                    ...
                    warp_att_reference_image = DAWarp(
                        cur_reference_image,
                        last_multi_self_offsets,
                        self_att_maps,
                        self.k,
        ...
        last_multi_self_offsets = F.interpolate(
            last_multi_self_offsets, reference_image.size()[2:], mode="bilinear"
        )
        last_multi_cross_offsets = F.interpolate(
        ...
        if use_light_en_de:
            ...
            warp_att_source_image = DAWarp(
                source_image, last_multi_cross_offsets, cross_att_maps, self.k, 64
            )
            warp_att_reference_image = DAWarp(
                reference_image, last_multi_self_offsets, self_att_maps, self.k, 64
            )
            result_tryon = self.lights_decoder(
                warp_att_source_image + warp_att_reference_image
            )
            ...
            warp_att_source_image = DAWarp(
                source_image, last_multi_cross_offsets, cross_att_maps, self.k, 3
            )
            warp_att_reference_image = DAWarp(
                reference_image, last_multi_self_offsets, self_att_maps, self.k, 3
            )
            result_tryon = warp_att_source_image + warp_att_reference_image
...
def get_model(
    model_path: str = "/opt/ml/input/038_model_all_256_part2.pt",
):
    """Model을 가져옵니다"""
...
def get_avatar(avatar_path: str = "./assets/daflow"):
    ...
    img = Image.open(osp.join(avatar_path, "images", f"{avatar_name}_0.jpg")).convert(
        "RGB"
    )
    img = transforms.Resize(384, interpolation=2)(img)
    img = transforms.ToTensor()(img)

    img_agnostic = Image.open(
        osp.join(avatar_path, "agnostic", f"{avatar_name}_0.jpg")
    ).convert("RGB")
    img_agnostic = transforms.Resize(384, interpolation=2)(img_agnostic)
    img_agnostic = transforms.ToTensor()(img_agnostic)
    img_agnostic = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(img_agnostic)

    pose = Image.open(
        osp.join(avatar_path, "skeletons", f"{avatar_name}_5.jpg")
    ).convert("RGB")
    pose = transforms.Resize(384, interpolation=2)(pose)
    pose = transforms.ToTensor()(pose)
    pose = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(pose)

    result = {"img": img, "agnostic": img_agnostic, "pose": pose}
