...
svc = bentoml.Service(
    "vton_daflow",
    runners=[vton_runner, openpose_runner, tracer_runner, fba_runner, parser_runner],
...
@svc.api(
    input=Multipart(cloth=Image(), avatar_path=Text()),
    output=Image(),
    route="/cloth-tryon",
)
async def predict_from_cloth(cloth, avatar_path):
    # cloth = preprocess.pre_resize(cloth)
    ...
    imgs = (
        (tryon_result.squeeze().permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5) * 255
    ).astype(np.uint8)
...
@svc.api(
    input=Multipart(part=Text(), cloth=Image(), human=Image()),
    output=Image(),
    route="/all-tryon",
)
def predict_from_cloth_human(part, cloth, human):
    ...
    # human = preprocess.pre_resize(human)
    # cloth = preprocess.pre_resize(cloth)
    ...
    cloth_img_result = (
        (cloth_img.squeeze().permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5) * 255
    ).astype(np.uint8)
