...
class DresscodeDataset(Dataset):
    ...
    def __init__(self, opt, phase, test_pairs=None):
        ...
        # 아직 잘 모르겠
        opt.offsets = [0]
        # Which segmentations to use for conditioning > 모든 sh파일들이 densepose만 써있다?
        ...
        if phase in {"test", "test_same"}:
            test_pairs = "train_pairs.txt"
            # test_pairs = "test_pairs_paired.txt"
        else:
            test_pairs = "train_pairs.txt"
        # test_pairs = "%s_pairs_unpaired.txt" % ("test" if phase in {"test", "test_same"} else "train") if test_pairs is None else test_pairs
        # test_pairs = "/home/benjamin/StyleTON/data/viton/viton_train_swap.txt"
        self.filepath_df = pd.read_csv(os.path.join(self.db_path, "upper_body", test_pairs), sep="\t", names=["poseA", "target"])
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(os.path.join(self.db_path, "upper_body", "images", df_row["poseA"]))
        # image = cv2.imread(os.path.join(self.db_path, "data", "image", df_row["poseA"]))
        print(os.path.join(self.db_path, "upper_body", "images", df_row["poseA"]))
        ...
        cloth_image = cv2.imread(os.path.join(self.db_path, "upper_body", "images", df_row["target"]))
        ...
        # load cloth labels
        cloth_seg = cv2.imread(os.path.join(self.db_path, "upper_body", "label_maps", df_row["poseA"].replace("_0", "_4").replace(".jpg", ".png")))
        ...
        body_seg = cv2.imread(os.path.join(self.db_path, "upper_body", "label_maps", df_row["poseA"].replace("_0", "_4").replace(".jpg", ".png")))
        ...
        densepose_seg = cv2.imread(os.path.join(self.db_path, "upper_body", "dense", df_row["poseA"].replace("_0", "_5").replace(".jpg", ".png")))
