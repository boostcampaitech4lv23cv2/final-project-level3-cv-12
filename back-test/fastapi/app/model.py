...
def get_model(model_path: str = "./assets/daflow/model.pt") -> SDAFNet_Tryon:
    """Model을 가져옵니다"""
...
def predict_from_image_byte(model: SDAFNet_Tryon, image_bytes: bytes, result: Dict[str, Any]) -> List[float]:
    cudnn.benchmark = True
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    transformed_image = _transform_image(image_bytes).to(device)

    # img = result['img'].cuda()
    img_agnostic = result['agnostic'].cuda() #Masked model image
    img_agnostic = img_agnostic.unsqueeze(0)
    pose = result['pose'].cuda()
    pose = pose.unsqueeze(0)
    cloth_img = transformed_image.cuda()
    ...
    print(tryon_result.shape)

    transf = transforms.ToPILImage()
    imgs = transf(tryon_result.squeeze())
    # tryon_result = Image.fromarray((tryon_result*255).astype(np.uint8))

    buffer = io.BytesIO()
    imgs.save(buffer, format='jpeg', quality=100)
    out = buffer.getvalue()
    return out
...
if __name__ == '__main__':
    ...
    # img = result['img'].cuda()
    img_agnostic = result['agnostic'].cuda()
    img_agnostic = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(img_agnostic)
    pose = result['pose'].cuda()
    pose = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(pose)

    cloth_img = Image.open('./assets/daflow/000000_1.jpg')
    cloth_img = transforms.Resize(192, interpolation=2)(cloth_img)
    cloth_img = transforms.ToTensor()(cloth_img)
    cloth_img = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(cloth_img)
    cloth_img = cloth_img.cuda()

    img_agnostic = img_agnostic.unsqueeze(0)
    pose = pose.unsqueeze(0)
    cloth_img = cloth_img.unsqueeze(0)

    ref_input = torch.cat((pose, img_agnostic), dim=1)

    print(ref_input.shape, cloth_img.shape, img_agnostic.shape)

    out = net(ref_input, cloth_img, img_agnostic).detach()
