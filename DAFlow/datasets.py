...
class DressCodeDataset(data.Dataset):
        ...
        for c in opt.dataset_imgpath:
            ...
            with open(list_path, 'r') as f:
                for line in f.readlines():
                    ...
                    img_names.append(img_name)
                    c_names.append(c_name)
                    dataroot_names.append(data_path)
    ...
    def __getitem__(self, index):
        ...
        data_path = self.dataroot_names[index]

        # load cloth image
        c = Image.open(osp.join(data_path, 'images', c_name)).convert('RGB')
        c = transforms.Resize(self.load_width, interpolation=2)(c)
        c = self.transform(c)
        
        # load person skeleton image
        pose_name = img_name.replace('_0.jpg', '_5.jpg')
        pose_rgb = Image.open(osp.join(data_path, 'skeletons', pose_name))
        pose_rgb = transforms.Resize(self.load_width, interpolation=2)(pose_rgb)
        pose_rgb = self.transform(pose_rgb)  
