...
def main():
    ...
    if uploaded_file:
        ...
        inf_image = Image.open(io.BytesIO(response.content))
        # label = response.json()["products"][0]["result"]
        # st.write(f'label is {label}')
        # out_image_bytes = response.json()
        # st.write(out_image_bytes)
        # out_image_np = out_image_bytes["products"][0]["result"]
        # out_image = Image.open(io.BytesIO(out_image_np))
        st.image(inf_image, caption='Virtual Avatar')
